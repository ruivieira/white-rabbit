...
Deno.test("Markov generation - basic functionality", async () => {
  const result = await generateCorpusMarkovAnswer("Once upon a time", 10);
...
Deno.test("Markov generation - max tokens respected", async () => {
  const maxTokens = 5;
  const result = await generateCorpusMarkovAnswer("The quick brown fox", maxTokens);
...
Deno.test("Markov generation - doesn't repeat first word from prompt", async () => {
  const prompt = "One upon a time";
  const result = await generateCorpusMarkovAnswer(prompt, 10);
  ...
  assert(
    firstGeneratedWord !== "one",
    `Generated text "${result.text}" should not start with "one" from prompt "${prompt}"`,
  );
});

Deno.test("Markov generation - various max token sizes", async () => {
  const testCases = [1, 3, 5, 10, 20, 50];

  for (const maxTokens of testCases) {
    const result = await generateCorpusMarkovAnswer("Test prompt", maxTokens);
    const tokens = result.text.trim().split(/\s+/);
...
Deno.test("Markov generation - empty/invalid prompts", async () => {
  // Empty prompt
  const emptyResult = await generateCorpusMarkovAnswer("", 10);
  assertExists(emptyResult.text);

  // Single character
  const singleCharResult = await generateCorpusMarkovAnswer("a", 10);
  assertExists(singleCharResult.text);

  // Non-existent words
  const nonsenseResult = await generateCorpusMarkovAnswer("xyzabc123", 10);
  assertExists(nonsenseResult.text);
});

Deno.test("Markov generation - null/undefined max tokens", async () => {
  const prompt = "Test with default limits";

  // null max tokens should use default (40)
  const nullResult = await generateCorpusMarkovAnswer(prompt, null);
  ...
  const undefinedResult = await generateCorpusMarkovAnswer(prompt, undefined);
...
Deno.test("Markov generation - zero/negative max tokens", async () => {
  const prompt = "Test edge cases";

  // Zero max tokens should use default
  const zeroResult = await generateCorpusMarkovAnswer(prompt, 0);
  ...
  const negativeResult = await generateCorpusMarkovAnswer(prompt, -5);
...
Deno.test("Markov generation - consistent token counting", async () => {
  const prompt = "The weather is beautiful today";
  const maxTokens = 8;

  // Run multiple times to ensure consistency
  for (let i = 0; i < 10; i++) {
    const result = await generateCorpusMarkovAnswer(prompt, maxTokens);
    const tokens = result.text.trim().split(/\s+/);
    ...
    assert(
      ...
      `Run ${
        i + 1
      }: Generated ${tokens.length} tokens, expected <= ${maxTokens}. Text: "${result.text}"`,
    );
  }
});

Deno.test("Markov generation - text formatting", async () => {
  const result = await generateCorpusMarkovAnswer("Test formatting", 15);

  // Should start with lowercase
  assert(result.text[0] === result.text[0].toLowerCase(), "First character should be lowercase");
...
Deno.test("Markov generation - prompt word continuation", async () => {
  ...
  for (const prompt of prompts) {
    ...
    assert(
      firstResultToken !== lastPromptToken,
      `For prompt "${prompt}", generated text "${result.text}" should not start with "${lastPromptToken}"`,
    );
  }
});

Deno.test("Markov generation - 95% max tokens requirement", async () => {
  const testCases = [
  ...
  for (const { prompt, maxTokens } of testCases) {
    ...
    assert(
      ...
      `Prompt "${prompt}": Should generate at least 95% of max_tokens. Got ${tokens.length}/${maxTokens} (${
